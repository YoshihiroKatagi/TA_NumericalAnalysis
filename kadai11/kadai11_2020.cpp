...
double  x = 0.4;                 // xの初期値設定
double  y = 0.4;                 // yの初期値設定
...
void calc_f_vector(double f_vector[number_of_equation], double x, double y)
{
  ...
  f_vector[0] = 4 * x - 4.2 * pow(x,3.0) + pow(x, 5.0) + y;
  f_vector[1] = x + 2.0 * y;
...
void calc_Jacobian(double Jacobian_matrix[number_of_equation][variables], double x, double y)
{
  ...
  Jacobian_matrix[0][0] = 4.0 - 12.6 * pow(x, 2.0) + 5.0 * pow(x, 4.0);
  Jacobian_matrix[0][1] = 1.0;
  Jacobian_matrix[1][0] = 1.0;
  Jacobian_matrix[1][1] = 2.0;
